...
public class SinaShare {
	...
	public void shareText(final String title,String content,final String imagePath){
		this.title = title;
		...
		if(!AccessTokenKeeper.readAccessToken(mContext).isSessionValid()){
        	...
        	WindowManager manage=(WindowManager)mContext.getSystemService(Context.WINDOW_SERVICE);
		     ...
		     Log.d("-----", "=======================" + screenWidth);
		    	 ...
		    	 final EditText textCenter = (EditText)view.findViewById(R.id.center_view);
		    	 Button buttonBottem = (Button)view.findViewById(R.id.botton_button);
	 ...
	 public void shareContent(final String con){
			    	 dialog=ProgressDialog.show(mContext, "",
								"正在分享...", true);
					StatusesAPI api = new StatusesAPI(AccessTokenKeeper.readAccessToken(mContext));
	...
	//绑定
		...
		public boolean isBundling(){
			if ((AccessTokenKeeper.readAccessToken(mContext).getToken() == null
					|| AccessTokenKeeper.readAccessToken(mContext).getToken().equals(""))) {
				return false;
			} else {
				return true;
			}
